...
function buildTodoDecorations(view: EditorView): DecorationSet {
  ...
  for (const { from, to } of view.visibleRanges) {
    tree.iterate({
      enter: (node) => {
        ...
        for (const marker of findTodoMarkers(text, visibleFrom)) {
          builder.add(marker.start, marker.end, Decoration.mark({ class: marker.className }));
        }
      },
      from,
      to,
  ...
  if (!hasCommentNodes) {
    for (const { from, to } of view.visibleRanges) {
      ...
      for (const marker of findTodoMarkers(text, from)) {
        builder.add(marker.start, marker.end, Decoration.mark({ class: marker.className }));
      }
    }
  }
...
export const THEME_EXTENSION = EditorView.theme({
  ...
  ".cm-scroller": {
    ...
    "&::-webkit-scrollbar-thumb:hover": {
      backgroundColor: "var(--text-secondary) !important",
    },
    ...
    fontFamily: "var(--font-mono)",
    scrollbarColor: "color-mix(in srgb, var(--foreground), transparent 70%) transparent",
    scrollbarWidth: "thin",
  },
