...
import { excludePath } from "@/server/utils/path-operations";
...
export function buildSuggestedPathsForEntry(params: {
  ...
  const currentPath = normalize(params.currentPath);
  const prioritizedSuggestions = [
    ...
    ...params.entry.hotspotSignals.map((item) => normalize(item.path)),
    ...params.entry.churnHotspots.map((item) => normalize(item.path)),
    ...params.entry.changeCoupling.flatMap((item) => [
      normalize(item.fromPath),
      normalize(item.toPath),
    ]),
...
export function buildNeighborPathsForEntry(params: {
  ...
  const normalizedCurrentPath = normalize(params.currentPath);
