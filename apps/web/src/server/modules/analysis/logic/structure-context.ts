...
export function aggregateEntryForPaths(paths: string[], context: StructureContext) {
  ...
  for (const path of normalizedPaths) {
    ...
    if (context.apiPaths.has(path)) {
      entry.apiPaths.push(path);
    }
    if (context.normalizedConfigInventory.includes(path)) {
      entry.configPaths.push(path);
    }
  ...
  for (const finding of context.aiResult.findings ?? []) {
    if (finding.evidence.some((item) => pathSet.has(normalize(item.path)))) {
...
export function buildStructureContext(
  ...
  const normalizedConfigInventory = metrics.configInventory.map((path) => normalize(path));
  ...
  for (const path of allInterestingPaths) {
    ...
    if (apiPaths.has(path)) {
      current.apiPaths.push(path);
    }
    if (normalizedConfigInventory.includes(path)) {
      current.configPaths.push(path);
    }
