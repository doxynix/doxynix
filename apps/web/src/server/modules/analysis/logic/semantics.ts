...
export function buildGroupKeySet(paths: string[], _apiPaths: Set<string>) {
  return uniq(paths.map((path) => ProjectPolicy.deriveGroupId(path)));
}
...
export function filterMeaningfulEntrypoints(paths: string[]) {
  const normalized = uniq(compact(Array.from(paths)).map((path) => normalize(path)));
...
function getStructureSeedScore(params: {
  ...
  const normalizedPath = normalize(params.path);
