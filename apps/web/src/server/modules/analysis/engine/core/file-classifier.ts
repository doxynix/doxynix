...
export function getFileScore(path: string): number {
  ...
  if (ProjectPolicy.isSensitive(lower)) {
    return FILE_CATEGORY_SCORING.sensitive;
  }
  if (ProjectPolicy.isLowSignalConfig(lower)) {
  ...
  if (ProjectPolicy.isDocsFile(lower)) {
    return FILE_CATEGORY_SCORING.docs;
  }
  if (ProjectPolicy.isBenchmarkFile(lower)) {
    return FILE_CATEGORY_SCORING.benchmarks;
  }
  if (ProjectPolicy.isTestFile(lower)) {
    return FILE_CATEGORY_SCORING.tests;
  }
  if (ProjectPolicy.isInfraFile(lower)) {
    return FILE_CATEGORY_SCORING.infrastructure;
  }
  if (ProjectPolicy.isToolingFile(lower)) {
    return FILE_CATEGORY_SCORING.tooling;
  }
  if (ProjectPolicy.isConfigFile(lower)) {
    return FILE_CATEGORY_SCORING.config;
  }
  if (ProjectPolicy.isApiPath(lower)) {
    return FILE_CATEGORY_SCORING.api;
  }
  if (ProjectPolicy.isRuntimeSource(lower)) {
    return FILE_CATEGORY_SCORING.runtimeSource;
  }

  if (ProjectPolicy.isFrontendComponent(lower)) {
    return FILE_CATEGORY_SCORING.runtimeSource;
  }
