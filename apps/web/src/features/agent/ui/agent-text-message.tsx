...
import { RepoCodeEditor } from "@/entities/repo/ui/repo-code-editor";

import { matchAlertPragma } from "../model/markdown-alerts";
...
function renderAlertOrFallback(
  ...
  const pragma = matchAlertPragma(fullText);

  if (pragma != null) {
    const alertConfig = configMap[pragma.type];
    const IconComponent = alertConfig.icon;

    const stripState = { charsToStrip: pragma.matchedLength };
