...
for (const baseDir of searchDirs) {
  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory() && entry.name.startsWith("tree-sitter-wasms")) {
        ...
        if (fs.existsSync(nestedNodeModules)) {
          fs.rmSync(nestedNodeModules, { force: true, recursive: true });
          console.log(
            `🧹 [clean:symlinks] Removed circular symlink directory: ${nestedNodeModules}`,
          );
          cleanedCount++;
        }
      }
    ...
    console.error(`⚠️ [clean:symlinks] Error reading ${bunCacheDir}:`, error?.message ?? error);
  }

  const directNested = path.join(baseDir, "node_modules", "tree-sitter-wasms", "node_modules");
  if (fs.existsSync(directNested)) {
    try {
      fs.rmSync(directNested, { force: true, recursive: true });
      console.log(`🧹 [clean:symlinks] Removed circular directory: ${directNested}`);
      cleanedCount++;
    } catch (error) {
      console.error(`⚠️ [clean:symlinks] Error removing ${directNested}:`, error?.message ?? error);
    }
  }
}
