...
>(({ className, isLoading, ...props }, ref) => (
  <div cmdk-input-wrapper="" className="flex items-center border-b px-3">
    ...
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        "placeholder:text-muted-foreground flex h-10 w-full rounded-xl bg-transparent py-3 pr-8 text-sm outline-none disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
      {...props}
    />
...
const CommandShortcut = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => {
  return (
    <kbd
      className={cn(
        "text-muted-foreground bg-muted ml-auto flex h-5 items-center rounded border px-1.5 text-xs tracking-widest",
        className
      )}
      {...props}
    />
