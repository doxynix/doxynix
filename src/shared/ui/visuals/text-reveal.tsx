    ...
    <div ref={targetRef} className={cn("relative z-0 h-[200dvh]", className)}>
      ...
      >
        <span
          ref={targetRef}
          className={
            "text-foreground/20 flex flex-wrap p-5 text-2xl font-bold md:p-8 md:text-3xl lg:p-10 lg:text-4xl xl:text-5xl"
          }
        >
          {words.map((word, i) => {
...
const Word: FC<WordProps> = ({ children, progress, range }) => {
  ...
  return (
    <span className="xl:lg-3 relative mx-1 lg:mx-1.5">
      <span aria-hidden="true" className="text-foreground/30 absolute">
        {children}
      </span>
