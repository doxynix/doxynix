...
async function getNextAfterApi() {
  ...
  async function runAsBackgroundTask(task: () => Promise<void>): Promise<void> {
    // In serverless environments, we prefer to await the task to ensure completion
    // before the request context is destroyed, unless explicitly running in a
    ...
    // Default to awaiting the task to ensure reliability across environments
    // and avoid race conditions with request context cleanup.
    try {
      await task();
    } catch (error) {
