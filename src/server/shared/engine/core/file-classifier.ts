...
export class FileClassifier {
  static isDependencyLockfile(path: string) {
    const lower = path.toLowerCase();
    return /(^|\/)(package-lock\.json|pnpm-lock\.yaml|yarn\.lock|bun\.lockb?|go\.sum|cargo\.lock|poetry\.lock|pipfile\.lock|mix\.lock|rebar\.lock|composer\.lock|gradle\.lockfile|\.terraform\.lock\.hcl)$/iu.test(
      lower
    );
  }

  static isLowSignalConfigFile(path: string) {
    const lower = path.toLowerCase();
    if (this.isDependencyLockfile(lower)) return true;
    if (/^prisma\/migrations\/[^/]+\/migration\.sql$/u.test(lower)) return true;
    if (/(^|\/)(gradlew|gradlew\.bat)$/iu.test(lower)) return true;
    if (/(\.|-)(min)\.(js|cjs|mjs|css)$/iu.test(lower)) return true;
    return false;
  }

  static isLikelyBarrelFile(path: string) {
    const lower = path.toLowerCase();
  ...
  static isUsefulComplexityCandidate(path: string) {
    const lower = path.toLowerCase();
    if (this.isPrimaryContourExcluded(lower)) return false;
    if (this.isConfigFile(lower) || this.isDocsFile(lower) || this.isToolingFile(lower))
      return false;
    if (this.isLowSignalConfigFile(lower)) return false;
    if (this.isLikelyBarrelFile(lower)) return false;
    return this.isArchitectureRelevant(lower) || this.isApiFile(lower);
  }
  ...
  static getScore(path: string): number {
    ...
    if (this.isLowSignalConfigFile(lower)) return 15;
