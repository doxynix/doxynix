...
export function buildNodeExplainPayloadFromContext(
  ...
  const sourcePaths = uniq([
    ...scopedPaths,
    ...signals.facts.flatMap((fact) => fact.evidence.map((item) => normalize(item.path))),
    ...signals.findings.flatMap((finding) => finding.evidence.map((item) => normalize(item.path))),
  ]).slice(0, 8);
  ...
  return {
    ...
    relationships: {
      ...
      riskTitles: uniq(signals.findings.map((finding) => finding.title)).slice(0, 5),
