...
function buildStructureNodeSummary(params: {
  ...
  const uniquePathsInGroup = uniq(params.entry.paths);
  const primaryKind = ProjectPolicy.getPrimarySemanticKind(params.entry.semanticCounts);
  const entrypointCount = uniq(params.entry.entrypointDetails.map((item) => item.path)).length;
  ...
  const riskCount = uniq(params.entry.riskTitles).length;
  const score =
    entrypointCount * 4 +
    apiCount * 3 +
    riskCount * 2 +
  ...
  return {
    ...
    label:
      params.nodeType === "group"
        ...
        : basename(params.path) || params.path,
    markers: {
      api: apiCount > 0,
