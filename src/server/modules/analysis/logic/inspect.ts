...
export function buildInspectPayload(params: {
  ...
  return {
      ...
      params.node.stats.apiCount === 0
        ...
        : [
            ...
            ...(params.entry.publicSurfacePaths.length > 0
              ? [
                  `Public surface paths: ${uniq(params.entry.publicSurfacePaths).slice(0, 3).join(", ")}.`,
                ]
              : []),
            ...(params.entry.graphNeighborPaths.length > 0
    ...
    configHints: uniq(params.entry.configPaths).slice(0, 5),
    dependsOn: uniq(params.outgoing),
    entrypointReason: params.entry.entrypointDetails[0]?.reason ?? null,
    factTitles: uniq(params.entry.factTitles).slice(0, 5),
    frameworkHints,
    gitHints,
    graphHints,
    hotspotHints,
