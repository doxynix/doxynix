...
export const PROJECT_POLICY_RULES = {
  ...
  security: {
    SECRETS: [
      { id: "stripe_key", pattern: /sk_live_[\da-z]{24}/i, title: "Stripe Live Key" },
      { id: "slack_token", pattern: /xox[abpr]-\d{12}/i, title: "Slack Token" },
      { id: "github_token", pattern: /ghp_[\da-z]{36}/i, title: "GitHub Personal Access Token" },
      { id: "google_api", pattern: /aiza[\w-]{35}/i, title: "Google API Key" },
      { id: "aws_key", pattern: /akia[\da-z]{16}/i, title: "AWS Access Key" },
      {
        id: "rsa_key",
        pattern: /-----begin\s+rsa\s+private\s+key-----/i,
        title: "Private RSA Key",
      },
      { id: "jwt_hardcoded", pattern: /eyjhbgcioi/i, title: "Potential Hardcoded JWT" },
      {
        id: "generic_secret",
        pattern: /(?:\bpassword\b|\bsecret\b|\bapi[_-]?key\b|\btoken\b)\s*=\s*["'][^"']{8,}["']/i,
    ...
    SQL_INJECTION: [
      ...
      {
        id: "sql_concat",
        pattern: /\bSELECT\s+[\w\s,.*()]+\bFROM\s+[\w\s.]+\bWHERE\s+[\w\s.]+\s*=\s*\${/i,
        title: "SQL Injection (String Concatenation)",
      },
    ],

    todoPatterns: [/\/\/\s*todo/i, /\/\/\s*fixme/i, /\/\/\s*xxx/i, /\/\/\s*hack/i],

    VULNERABILITIES: [
      // JS/TS
      { id: "js_eval", pattern: /\beval\(/i, title: "Dynamic Code Execution (eval)" },
      {
        id: "js_inner_html",
      ...
      },
      {
        ...
        title: "Process Command Injection",
      },

      // Python
      {
        id: "py_rce",
        pattern: /\b(?:os\.system|subprocess\.(?:popen|run|call))\s*\(/i,
        title: "Python OS Command Injection",
      },
      {
        id: "py_pickle",
        pattern: /pickle\.load\(|yaml\.unsafe_load\(/i,
        title: "Insecure Deserialization (RCE)",
      },

      // Go / Rust / C++
      { id: "unsafe_block", pattern: /\bunsafe\s*{/i, title: "Unsafe Code Block" },
      {
        id: "cpp_system",
        pattern: /\bsystem\(|\bpopen\(|\bpassthru\(/i,
      ...
      {
        id: "java_exec",
        pattern: /runtime\.getruntime\(\)\.exec\(/i,
        title: "Java Command Execution",
      },
      {
        id: "php_eval",
        pattern: /\b(?:shell_exec|exec|passthru|eval)\(/i,
